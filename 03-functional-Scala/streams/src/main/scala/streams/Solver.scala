...
//      println(steps)
//      steps = steps + 1
      //initial ++ from(more, (explored.toList ++ (more map (_._1)).toList).toSet) 
      //val newExplored = more.foldLeft(explored) { case (s, (b, _)) => s + b }
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] = //???
    pathsFromStart filter (i => done(i._1))
//    for { p@(b,ms) <- pathsFromStart if done(b) } yield p
