...
abstract class TweetSet {
   ...
   */
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet //= filter(p) union acc
   ...
   * and be implemented in the subclasses?
   */
   def union(that: TweetSet): TweetSet //= //??? //((left union right) union other) incl elem
...
class Empty extends TweetSet {
  ...
  def tail: Nothing = throw new Error("Empty.tail")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet  = {
      if (p(head)) tail.filterAcc(p, acc.incl(head))
      else tail.filterAcc(p, acc)
      
    }
    
  def mostRetweeted: Tweet = {
    val lmr = if (left.isEmpty) elem else left.mostRetweeted
    val rmr = if (right.isEmpty) elem else right.mostRetweeted
    val bmr = if (lmr.retweets > rmr.retweets) lmr else rmr
    if (bmr.retweets > elem.retweets) bmr else elem

//    if (!(left.isEmpty) && (elem.retweets < left.mostRetweeted.retweets))
//      if (left.mostRetweeted.retweets < right.mostRetweeted.retweets) right.mostRetweeted
...
object GoogleVsApple {
  ...
  def textOnList(l: List[String]): (Tweet => Boolean) = tw => l.exists(x => tw.text.contains(x))

  //lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tw => google.exists(x => tw.text.contains(x)))
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(textOnList(google))
